...
public class Energy1DSolver {
	...
	public Energy1DSolver(String soilHeatCapacityModel, String soilThermalConductivityModel, String topBCType,
	...
	public void solve(double topBC, double bottomBC, double rain, double  shortWave, double wind,  String inCurrentDate, double timeDelta) {
		...
		System.out.println("ENERGY 1D "+inCurrentDate);
		...
		if(Variables.thetas[NUM_CONTROL_VOLUMES-1]>0.0) {
			for(int i = 0; i < NUM_CONTROL_VOLUMES; i++) {
				if( i == 0 ) {
					...
					mainDiagonal[i] = Variables.heatCapacities[i]*dx[i] + bottomBoundaryCondition.mainDiagonal(-999, lambdaP, lambdaM, spaceDelta[i+1], spaceDelta[i], timeDelta, delta) 
					+ timeDelta*densityWater*cwWater*( 0.5*Variables.darcyVelocities[i+1] + 0.5*Math.abs(Variables.darcyVelocities[i+1]) - (0.5*Variables.darcyVelocities[i] - 0.5*Math.abs(Variables.darcyVelocities[i])) );

					upperDiagonal[i] = bottomBoundaryCondition.upperDiagonal(-999, lambdaP, lambdaM, spaceDelta[i+1], spaceDelta[i], timeDelta, delta)
							+ timeDelta*densityWater*cwWater*( 0.5*Variables.darcyVelocities[i+1] - 0.5*Math.abs(Variables.darcyVelocities[i+1]) );

					rhss[i] = Variables.internalEnergiesOld[i] +  Variables.heatCapacities[i]*dx[i]*273.15 + bottomBoundaryCondition.rightHandSide(bottomBC, lambdaP, lambdaM, spaceDelta[i+1], spaceDelta[i], timeDelta, delta)
					- timeDelta*densityWater*cwWater*( -0.5*Variables.darcyVelocities[i]*(bottomBC) - 0.5*Math.abs(Variables.darcyVelocities[i])*(bottomBC) )
					...
					lowerDiagonal[i] = topBoundaryCondition.lowerDiagonal(-999, lambdaP, lambdaM, -999, spaceDelta[i], timeDelta, delta)
							//aggiunta oggi
							+ timeDelta*densityWater*cwWater*( -0.5*Variables.darcyVelocities[i] - 0.5*Math.abs(Variables.darcyVelocities[i]) );

					mainDiagonal[i] = Variables.heatCapacities[i] + topBoundaryCondition.mainDiagonal(-999, lambdaP, lambdaM, spaceDelta[i], spaceDelta[i], timeDelta, delta)
					+ timeDelta*densityWater*cwWater*( +0.5*(-rain) + 0.5*Math.abs(-rain) - 0.5*Variables.darcyVelocities[i] + 0.5*Math.abs(Variables.darcyVelocities[i]) );

					upperDiagonal[i] = topBoundaryCondition.upperDiagonal(topBC, lambdaP, lambdaM,  -999, spaceDelta[i], timeDelta, delta);

					rhss[i] = Variables.internalEnergiesOld[i] + Variables.heatCapacities[i]*273.15 + topBoundaryCondition.rightHandSide(topBC, lambdaP, lambdaM,  spaceDelta[i], spaceDelta[i-1], timeDelta, delta)
					...
					lowerDiagonal[i] = -lambdaM*timeDelta/spaceDelta[i] + timeDelta*densityWater*cwWater*( -0.5*Variables.darcyVelocities[i] - 0.5*Math.abs(Variables.darcyVelocities[i]) );

					mainDiagonal[i] = Variables.heatCapacities[i]*dx[i] + lambdaM*timeDelta/spaceDelta[i] + lambdaP*timeDelta/spaceDelta[i+1] 
							+ timeDelta*densityWater*cwWater*( 0.5*Variables.darcyVelocities[i+1] + 0.5*Math.abs(Variables.darcyVelocities[i+1]) - 0.5*Variables.darcyVelocities[i] + 0.5*Math.abs(Variables.darcyVelocities[i]) );

					upperDiagonal[i] = -lambdaP*timeDelta/spaceDelta[i+1] + timeDelta*densityWater*cwWater*( 0.5*Variables.darcyVelocities[i+1] - 0.5*Math.abs(Variables.darcyVelocities[i+1]) );
			...
			for(int i = 0; i < NUM_CONTROL_VOLUMES-1; i++) {
				if( i == 0 ) {
					...
					mDiagonal[i] = Variables.heatCapacities[i]*dx[i] + bottomBoundaryCondition.mainDiagonal(-999, lambdaP, lambdaM, spaceDelta[i+1], spaceDelta[i], timeDelta, delta) 
					+ timeDelta*densityWater*cwWater*( 0.5*Variables.darcyVelocities[i+1] + 0.5*Math.abs(Variables.darcyVelocities[i+1]) - (0.5*Variables.darcyVelocities[i] - 0.5*Math.abs(Variables.darcyVelocities[i])) );

					uDiagonal[i] = bottomBoundaryCondition.upperDiagonal(-999, lambdaP, lambdaM, spaceDelta[i+1], spaceDelta[i], timeDelta, delta)
							+ timeDelta*densityWater*cwWater*( 0.5*Variables.darcyVelocities[i+1] - 0.5*Math.abs(Variables.darcyVelocities[i+1]) );

					r[i] = Variables.internalEnergiesOld[i] +  Variables.heatCapacities[i]*dx[i]*273.15 + bottomBoundaryCondition.rightHandSide(bottomBC, lambdaP, lambdaM, spaceDelta[i+1], spaceDelta[i], timeDelta, delta)
					- timeDelta*densityWater*cwWater*( -0.5*Variables.darcyVelocities[i]*(bottomBC) - 0.5*Math.abs(Variables.darcyVelocities[i])*(bottomBC) )
					+ timeDelta*densityWater*cwWater*( Variables.darcyVelocities[i+1]*273.15 - Variables.darcyVelocities[i]*273.15 );
					...
					lambdaP = interfaceThermalConductivity.compute(Variables.lambdas[i],Variables.lambdas[i],dx[i],dx[i]);	
					lambdaM = interfaceThermalConductivity.compute(Variables.lambdas[i],Variables.lambdas[i-1],dx[i],dx[i-1]);
					lDiagonal[i] = -lambdaM*timeDelta/spaceDelta[i] + timeDelta*densityWater*cwWater*( -0.5*Variables.darcyVelocities[i] - 0.5*Math.abs(Variables.darcyVelocities[i]) );

					mDiagonal[i] = Variables.heatCapacities[i]*dx[i] + lambdaM*timeDelta/spaceDelta[i] + lambdaP*timeDelta/spaceDelta[i] 
					+ timeDelta*densityWater*cwWater*( +0.5*(Variables.darcyVelocities[i+1]) + 0.5*Math.abs(Variables.darcyVelocities[i+1]) - 0.5*Variables.darcyVelocities[i] + 0.5*Math.abs(Variables.darcyVelocities[i]) );

					uDiagonal[i] = 0.0;//topBoundaryCondition.upperDiagonal(topBC, lambdaP, lambdaM,  -999, spaceDelta[i], timeDelta, delta);

					r[i] = Variables.internalEnergiesOld[i] + Variables.heatCapacities[i]*dx[i]*273.15  + lambdaP*timeDelta/spaceDelta[i]*topBC
					- timeDelta*densityWater*cwWater*( 0.5*(Variables.darcyVelocities[i+1])*(topBC) - 0.5*Math.abs((Variables.darcyVelocities[i+1]))*(topBC) )
					+ timeDelta*densityWater*cwWater*( Variables.darcyVelocities[i+1]*273.15 - Variables.darcyVelocities[i]*273.15 ) + timeDelta*shortWave;
					//flussi di energia alla superficie
					//+ timeDelta*shortWave + timeDelta*epsilon*epsilonAir*5.67*Math.pow(10, -8)*Math.pow(topBC, 4)  + timeDelta*epsilon*5.67*Math.pow(10, -8)*Math.pow(Variables.temperatures[i], 4)
					//+ timeDelta*densityAir*cpAir*wind/ra*topBC;
					...
					mDiagonal[i] = Variables.heatCapacities[i]*dx[i] + lambdaM*timeDelta/spaceDelta[i] + lambdaP*timeDelta/spaceDelta[i+1] 
							+ timeDelta*densityWater*cwWater*( 0.5*Variables.darcyVelocities[i+1] + 0.5*Math.abs(Variables.darcyVelocities[i+1]) - 0.5*Variables.darcyVelocities[i] + 0.5*Math.abs(Variables.darcyVelocities[i]) );

					uDiagonal[i] = -lambdaP*timeDelta/spaceDelta[i+1] + timeDelta*densityWater*cwWater*( 0.5*Variables.darcyVelocities[i+1] - 0.5*Math.abs(Variables.darcyVelocities[i+1]) );
		...
		if(Variables.thetas[NUM_CONTROL_VOLUMES-1]>0.0) {
			Variables.errorEnergy = internalEnergyNew - internalEnergy - timeDelta*( -densityWater*cwWater*( 0.5*(-rain)*(topBC-273.15 + Variables.temperatures[NUM_CONTROL_VOLUMES -1]-273.15) - 0.5*Math.abs(-rain)*(topBC-273.15 - Variables.temperatures[NUM_CONTROL_VOLUMES -1]+273.15) )
					...
					+ Variables.lambdas[NUM_CONTROL_VOLUMES -1]/spaceDelta[NUM_CONTROL_VOLUMES -2]*(topBC-Variables.temperatures[NUM_CONTROL_VOLUMES -1]) - Variables.lambdas[0]/spaceDelta[0]*(Variables.temperatures[0]-bottomBC ) )
					- timeDelta*shortWave;
			...
			Variables.errorEnergy = internalEnergyNew - (internalEnergy-Variables.internalEnergiesOld[NUM_CONTROL_VOLUMES-1]) - timeDelta*( -densityWater*cwWater*( 0.5*(Variables.darcyVelocities[NUM_CONTROL_VOLUMES -1])*(topBC-273.15 + Variables.temperatures[NUM_CONTROL_VOLUMES -2]-273.15) - 0.5*Math.abs(Variables.darcyVelocities[NUM_CONTROL_VOLUMES -1])*(topBC-273.15 - Variables.temperatures[NUM_CONTROL_VOLUMES -2]+273.15) )
					- densityWater*cwWater*( -0.5*Variables.darcyVelocities[0]*(Variables.temperatures[0]-273.15+bottomBC-273.15) + 0.5*Math.abs(Variables.darcyVelocities[0])*(Variables.temperatures[0]-273.15-bottomBC+273.15) )
					+ Variables.lambdas[NUM_CONTROL_VOLUMES -1]/spaceDelta[NUM_CONTROL_VOLUMES -2]*(topBC-Variables.temperatures[NUM_CONTROL_VOLUMES -2]) - Variables.lambdas[0]/spaceDelta[0]*(Variables.temperatures[0]-bottomBC ) )
					- timeDelta*shortWave;
		}
		System.out.println();
